...
#U-Net SeResNext101 + CBAM + hypercolumns + deep_supervision
class UNET_SERESNEXT101(nn.Module):
    def __init__(self, deep_supervision, clfhead, clf_threshold, pretrained=True):
        super().__init__()
        ...
        if pretrained:
            seresnext101.load_state_dict(torch.load('models/pretrained_models/se_resnext101_32x4d-3b2fe3d8.pth'))
...
def build_model(model_name, deep_supervision, clfhead, clf_threshold, pretrained):
    if model_name=='seresnext101':
        model = UNET_SERESNEXT101(deep_supervision, clfhead, clf_threshold, pretrained)
    return model